...
public class GeneratePDFBoxplotsWithR<Result> implements ExperimentComponent {
    ...
    public void run() throws IOException {
        ...
        for (GenericIndicator<? extends Solution<?>> indicator : experiment.getIndicatorList()) {
            createTimesBoxplot(indicator.getName());
            createD4STimesBoxplot(indicator.getName());
        ...
        createD4STimesBoxplot("TIMES");
        createD4STimesBoxplot("TIMES10s");
        createD4STimesBoxplot("TIMES30s");
    }
    ...
    public void createTimesBoxplot(String name) throws IOException {
        ...
        String rFileName = rDirectoryName + "/" + name + ".Boxplot" + ".R";
        FileWriter os = new FileWriter(rFileName, false);
        os.write("pdf(\""
                + finalPath
                + "/R/"
        ...
        os.write("algs<-c(\"NSGAII\", \"NSGAII-D4S\")" + "\n");
    ...
    public void createD4STimesBoxplot(String name) throws IOException {
        String rDirectoryName = experiment.getExperimentBaseDirectory() + "/" + DEFAULT_R_DIRECTORY;
        File rOutput;
        String finalPath = experiment.getExperimentBaseDirectory().replaceAll("\\\\", "/");

        String rFileName = rDirectoryName + "/" + name + "D4S.Boxplot" + ".R";
        FileWriter os = new FileWriter(rFileName, false);
        os.write("pdf(\""
                + finalPath
                + "/R/"
        ...
        os.write("algs<-c(");
        for (int i = 1; i < experiment.getAlgorithmList().size() - 1; i++) {
            os.write("\"" + experiment.getAlgorithmList().get(i).getAlgorithmTag() + "\",");
        } // for
        os.write("\"" + experiment.getAlgorithmList().get(experiment.getAlgorithmList().size() - 1).getAlgorithmTag() + "\")" + "\n");
