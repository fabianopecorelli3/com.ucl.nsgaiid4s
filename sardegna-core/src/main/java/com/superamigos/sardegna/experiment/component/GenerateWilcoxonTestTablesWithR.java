...
/**
 * This class generates a R script that computes the Wilcoxon Signed Rank Test
 * and generates a Latex script that produces a table per quality indicator
 * containing the pairwise comparison between all the algorithms on all the
 * solved problems.
 *
...
public class GenerateWilcoxonTestTablesWithR<Result> implements ExperimentComponent {
    ...
    public GenerateWilcoxonTestTablesWithR(Experiment<?, Result> experimentConfiguration, FileManager fileManager, String hdfsPath) {
    ...
    private void printEndLatexCommands(String rFileName, String latexFileName) throws IOException {
        OutputStream os = filemanager.openW(hdfsPath, rFileName, true);
