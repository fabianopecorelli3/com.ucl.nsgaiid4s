...
/*    new ComputeQualityIndicators<>(experiment).run();
  ...
  static List<ExperimentAlgorithm<DoubleSolution, List<DoubleSolution>>> configureAlgorithmList(
    ...
    SparkConf sparkConf = new SparkConf().setAppName("Sardegna").setMaster("local[2]").set("spark.executor.memory", "1g"); //TODO capire che significa
    JavaSparkContext sparkContext = new JavaSparkContext(sparkConf);
      for (int i = 0; i < problemList.size(); i++) {
        ModifiedAbstractGeneticAlgorithm<DoubleSolution,List<DoubleSolution>> algorithm = new SardegnaNSGAIIBuilder<>(
                problemList.get(i).getProblem(),
                new SardegnaCrossover(1.0, 5),
